...
def main():
    ...
    future_days = data_testing.head(7)  # Fixed horizon to 7 days
    ...
    final_df = pd.concat([past_100_days, data_testing.head(7)], ignore_index=True)
    final_df['Date'] = pd.to_datetime(final_df['Date'])  # Convert 'Date' column to datetime
    final_df['Predicted'] = np.nan
    final_df['Predicted'][100:100 + 7] = y_predicted_custom.flatten()

    # Plot Predicted v/s True with fixed horizon
    y_test_custom = y_test * scale_factor_custom
    plot_predicted_vs_true(final_df, y_predicted_custom.flatten(), 'Predicted v/s True for 7-day Horizon')
