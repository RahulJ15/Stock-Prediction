...
def add_user_stock(username, stock):
    ...
    if stock in fetch_user_stocks(username):
        st.error(f"{stock} is already in your list")
        return
...
def app_content(username):
    ...
    for stock in user_stocks:
        ...
        if cols[1].button("Remove", key=f"remove_{stock}"):
            ...
            st.sidebar.success(f"Removed {stock} from your list")
            st.experimental_rerun()

    new_stock = st.sidebar.text_input("Add a new stock to your list")
    if st.sidebar.button("Add Stock"):
        add_user_stock(username, new_stock)
        st.sidebar.success(f"Added {new_stock} to your list")
    ...
    st.write("This chart now includes the true prices, the model's predictions for the existing data, and the scaled-down predictions for the next 7 days.")
